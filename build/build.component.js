...
const runBuild = async () => {
  ...
  pkgs.push({name: "@element-plus/alert"})
  ...
  async function build(name) {
    if (!name) return
    console.log(path.resolve(__dirname, `../packages/${name.split('@element-plus/')[1]}/index.js`),'-----------')
    const inputOptions = {
      input: path.resolve(__dirname, `../packages/${name.split('@element-plus/')[1]}/index.js`),
      plugins: [
        nodeResolve(),
        css({
          output:"alert.css"

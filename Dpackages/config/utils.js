...
export class Colors{
	...
	rgbToHexadecimal(rgb){
		if(rgb&&rgb.length==3){
			return '#'
			+`${this.#cArray[Math.floor(rgb[0]/16)]}${this.#cArray[rgb[0]%16]}`
			+`${this.#cArray[Math.floor(rgb[1]/16)]}${this.#cArray[rgb[1]%16]}`
			+`${this.#cArray[Math.floor(rgb[2]/16)]}${this.#cArray[rgb[2]%16]}`
		}else return ''
	}
	hslToRgb(H, S, L){
		var R, G, B;
